...
The inverse Gamma or special cases of the Gamma distribution (where restrictions have been placed on $\alpha$ and $\beta$) could be considered in future research. Regarding the latter option, at this early stage of modeling the FLE the Gamma function was chosen for its greater flexibility. Though it should be noted that the Gamma model could also be sanctioned for this flexibility, should a specific case be the better fit. 
...
As before, priors are placed on the mean ($\bar{x}$) and the standard deviation ($s$) of $\theta_i$, which is assumed to be normally distributed (matching the model assumptions of the unconstrained model without a spike).
...
(1)~\theta_i|(z_i=1)&~ \sim \mathcal{N}(\mu_\theta, \sigma_\theta) \\ \\
&~\mu_\theta \sim \log \mathcal{N}(a_\mu, b_\mu) \\ 
&~\sigma_\theta = -\mu_\theta / \Phi^{-1}(0.5 \; p(\theta_i<0)) \\
&~p(\theta_i<0) = \Phi(p^{*}_{\theta_i<0}) \\
&~p^{*}(\theta_i<0) \sim \mathcal{N}(a_p, b_p) \\ \\
...
\end{aligned}
$$
In this model $\omega$ is the spike weight, i.e. the proportion of non-responders with an $\theta_i = 0$. p_negative_slab is conceptualized as the proportion of non-responders and the proportion of negative effects, i.e. $P(\theta_i \le 0) = \omega + P(\theta_i <0)$. 

```{r}
#| label: prior-spike-and-slab 

p_negative_slab_prior <- rnorm(1e5, mean = -2/3, sd = 2/3) |>
  pnorm() |>
  (\(x) x / 2)()

omega_prior <- rnorm(1e5, mean = 0, sd = 0.5) |>
  pnorm() |>
  (\(x) p_negative_slab_prior*x)()

p_negative_slab_prior |>
  logspline::logspline(lbound = 0, ubound = 0.5) |>
  (
    \(fit) {
      curve(
...
post_populPara_spike <- tidybayes::gather_draws(
...
```{r}
#| label: tbl-compare-estimates-spike
#| tbl-cap: "Comparison of Bayesian posterior means of the spike model and MLE estimates."

data.frame(
  parameter = c("mu_theta", "sigma_theta", "sigma_epsilon", "P(theta_i < 0)", "P(theta_i <= 0)", "P(theta_i = 0)")
  , spike = fle_spike_test_bayes |>
    summary(pars = c("mu_theta", "sigma_theta", "sigma_epsilon", "p_negative_theta", "p_negative_spike", "omega")) |>
    (\(x) x$summary)() |>
    as.data.frame() |>
    dplyr::pull("mean")
  , mle = c(
...
## Reparameterization of truncated normal

```{r}
 #| label: truncated-transformation

# Testing whether currently implied prior for CV = s/m is in bounds of 0 < CV < sqrt(pi/2-1)

cv_trunc_prior <- data.frame(cv = sd_prior_truncn/mean_prior_truncn) 
max(cv_trunc_prior) <= sqrt(pi/2-1)
cv_trunc_prior|>
  ggplot() +
  aes(x = cv) +
  lims(x = c(0,5)) +
  geom_histogram() +
  labs(
    x = "Implied CV of truncated normal"
    , y = "Density"
  ) +
  geom_vline(xintercept = sqrt(pi/2-1), color = "red", linetype = 1) +
  papaja::theme_apa(base_size = 16)
