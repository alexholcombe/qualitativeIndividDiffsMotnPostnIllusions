...
# Prior settings
## Prior settings for $\mu_{\theta}$ (mean of $\theta_i$)

In the current truncated normal model priors have been set on the model parameters $\mu$ and $\sigma$. As the mass of the distribution has been cut and restricted to the positive domain, these do not correspond to the mean and the standard deviation of the distribution, as is the case with the regular normal distribution. Reparametrisation of these priors is still WIP, priors will be added to the plot at a later time. 
...
data.frame(priors = c(mu_prior
                      ,mean_prior_log
                      ,mu_prior_common)
                      ,model = rep(c("unconstrained","dominance (log-normal)","common"), each = 1e5)) |>
  ggplot() +
...
## Prior settings for $\sigma_{\theta}$ (standard deviation of $\theta_i$)
...
data.frame(priors = c(sigma_prior
                      ,sd_prior_log
                      ,sigma_prior_average)
                      ,model = rep(c("unconstrained","dominance (log-normal)","average null"), each = 1e5)) |>
  ggplot() +
  aes(x = priors, fill = model) +
  lims(x = c(0,5)) +
  geom_density(alpha = 0.2, linewidth = 0.5) +
