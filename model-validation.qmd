...
# Validation of log-normal model

```{r}
#| label: log-normal-data simulation
...
set.seed(123)
theta <- rlnorm(n_subjects, meanlog = mu_theta, sdlog = sigma_theta)
y <- rnorm(n_subjects*n_trials, mean = theta[id], sd = sigma_epsilon)

sim_data_logN <- data.frame(id = id, y = y)
...
stan_model <- readLines("./models/pse-dominance-alternative.stan") |>
  paste(collapse = "\n")
...
data.frame(
 ...
 , truth = c(mu_theta, sigma_theta, sigma_epsilon)
